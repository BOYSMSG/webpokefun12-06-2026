...
export default function EventsPage() {
  ...
  const handleAction = async (eventId: string, action: 'APPLY' | 'APPROVE', targetUsername?: string) => {
    const loadingKey = `${eventId}-${action}-${targetUsername || 'me'}`;
    ...
    try {
      const res = await fetch('/api/events', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventId, action, targetUsername })
      });
      const data = await res.json();
      if (data.success) {
        alert(data.message);
    ...
    return (
      <div key={t._id} style={{ background: "rgba(30, 34, 39, 0.7)", borderRadius: "12px", padding: "25px", marginBottom: "20px", border: "1px solid #333", boxShadow: "0 8px 30px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "15px" }}>
          <div>
            <h3 style={{ margin: 0, color: "#9b59b6", fontSize: "1.5rem", fontWeight: 800 }}>
              <i className="fa-solid fa-calendar-star"></i> {t.name}
            </h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px", marginBottom: "20px", fontSize: "0.9rem" }}>
          ...
          <div style={{ background: "#222", padding: "12px", borderRadius: "8px" }}>
            <div style={{ color: "#888", marginBottom: "5px" }}><i className="fa-solid fa-users"></i> Slots</div>
        ...
        {t.rules && (
          <div style={{ background: "rgba(255,255,255,0.05)", padding: "15px", borderRadius: "8px", marginBottom: "20px" }}>
            <div style={{ color: "#fff", fontWeight: "bold", marginBottom: "5px" }}>Rules / Details</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {t.status === 'UPCOMING' && (
            <button 
              ...
              style={{ 
                background: isApproved ? "#2ecc71" : hasApplied ? "#f39c12" : "#9b59b6", 
                color: "#fff", 
            ...
            >
              {!session?.user ? "Login to Apply" : isApproved ? "Approved! See you there" : hasApplied ? "Application Pending" : t.approvedPlayers.length >= t.maxPlayers ? "Event Full" : "Apply to Join"}
            </button>
          )}
        </div>
  ...
  return (
    <>
      <Head>
        <title>Events - Pokefun</title>
      </Head>
      
      <div className="container" style={{ paddingTop: "40px", paddingBottom: "40px", maxWidth: "800px", margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px" }}>
          <div>
            <h1 style={{ fontSize: "2.5rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: "1px", color: "#9b59b6", margin: 0 }}>
              Server Events
            </h1>
            <p style={{ color: "#a3a3a3", margin: "5px 0 0 0" }}>Join special server-wide events and mini-games!</p>
          </div>
          {isAdmin && (
            <button 
              onClick={() => setShowCreateModal(true)}
              style={{ background: "#9b59b6", color: "#fff", border: "none", padding: "10px 20px", borderRadius: "8px", fontWeight: "bold", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" }}
            >
        ...
        {loading ? (
          <div style={{ textAlign: "center", color: "#a3a3a3", padding: "40px" }}>Loading Events...</div>
        ) : events.length === 0 ? (
          <div style={{ textAlign: "center", background: "#1c1f21", padding: "40px", borderRadius: "12px", color: "#888" }}>
            <i className="fa-solid fa-calendar-star" style={{ fontSize: "3rem", marginBottom: "15px", opacity: 0.5 }}></i>
            <h2>No active events</h2>
            <p>Stay tuned! Our staff is planning the next big event.</p>
          </div>
        ) : (
      ...
      {showCreateModal && isAdmin && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.8)", zIndex: 1000, display: "flex", justifyContent: "center", alignItems: "center", padding: "20px" }}>
          <div style={{ background: "#222", borderRadius: "12px", width: "100%", maxWidth: "500px", border: "2px solid #9b59b6" }}>
            <div style={{ padding: "20px", borderBottom: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ margin: 0, color: "#fff", fontSize: "1.5rem", fontWeight: "bold" }}><i className="fa-solid fa-calendar-star"></i> Create Event</h2>
            ...
            <form onSubmit={handleCreateEvent} style={{ padding: "20px", maxHeight: "70vh", overflowY: "auto" }}>
              <div style={{ marginBottom: "15px" }}>
                <label style={{ display: "block", color: "#a3a3a3", marginBottom: "5px", fontSize: "0.9rem" }}>Event Name *</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px", marginBottom: "20px" }}>
                <div>
                  <label style={{ display: "block", color: "#a3a3a3", marginBottom: "5px", fontSize: "0.9rem" }}>Max Slots *</label>
              ...
              <button type="submit" disabled={createLoading} style={{ width: "100%", background: "#9b59b6", color: "#fff", border: "none", padding: "12px", borderRadius: "6px", fontWeight: "bold", fontSize: "1.1rem", cursor: createLoading ? "not-allowed" : "pointer" }}>
