...
export default function FakemonDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const unwrappedParams = use(params);
  const mon = fakemonsData.find(m => m.id === unwrappedParams.id);

  if (!mon) {
    return (
      <div className="inner" style={{ padding: "100px 20px", textAlign: "center" }}>
        <h2>Fakemon not found.</h2>
        <Link href="/wiki/fakemons" style={{ color: "var(--ghost-accent-color)" }}>Return to Dex</Link>
      </div>
    );
  }
  ...
  return (
    <div className="inner" style={{ padding: "60px 0", maxWidth: "1000px", margin: "0 auto" }}>
      <Link href="/wiki/fakemons" style={{ color: "var(--ghost-accent-color)", fontWeight: "bold", display: "block", marginBottom: "30px" }}>
        &larr; Back to Dex
      </Link>
        ...
        <div style={{ flex: "1 1 350px" }}>
          <div className="fakemon-card" style={{ padding: "40px", position: "relative" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "center", marginBottom: "30px" }}>
              {mon.allImages && mon.allImages.length > 0 ? (
                mon.allImages.map((imgSrc: string, idx: number) => (
                  <div key={idx} style={{ width: "150px", height: "150px", background: "rgba(0,0,0,0.05)", borderRadius: "10px", padding: "10px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <img 
                      src={imgSrc} 
                      alt={`${mon.name} form ${idx}`}
                ...
                <div style={{ width: "200px", height: "200px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                   <img 
                     src={getFakemonImage(mon.name, mon.id)} 
                     alt={mon.name}
                     onError={(e) => { e.currentTarget.style.display = 'none'; }}
                     style={{ width: "100%", height: "100%", objectFit: "contain" }}
                   />
            ...
            <h1 style={{ fontSize: "2.5rem", fontWeight: 800, margin: "20px 0" }}>{capitalize(mon.name)}</h1>
